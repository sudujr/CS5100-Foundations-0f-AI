...
def expectedUtility(parentDict, varDict, parentDictList):
    ...
    while i < len(utilityKeylist):
        ...
        res += jointProbabilityCalculatorWithHiddenVariables(input) * utilityValue[i]
        i += 1
...
# Compute expected utility of all the possible value taken by qDict nd store it in expectedUtilityDict
...
def maximumExpectedUtilityWithConditional(qDict, evDict, qList):
    ...
    for index , value in enumerate(vDictKeylist):
        ...
        sign = "".join(qDict.values())
        evDict.update(qDict)
        s_add = jointProbabilityCalculatorWithHiddenVariables(evDict)
        val = expectedUtility(parentDict, evDict, parentDictList)
        expectedUtilityDict[sign] = val/s_add
    # TODO section

...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        bool = False
        for effects in actionA.effects:
            if ~effects in actionB.effects:
                return True
        return False
    ...
    def _interference(self, actionA, actionB):
        ...
        # TODO: implement this function
        for effects in actionA.effects:
            if  ~effects in actionB.preconditions:
                return True
        for effects in actionB.effects:
            if ~effects in actionA.preconditions:
                return  True
        return False
        raise NotImplementedError
...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        bools = [True if self.parent_layer.is_mutex(a1,a2) else False for a1 in actionA for a2 in actionB]
        return all(bools)
        raise NotImplementedError
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        for i,layer in enumerate(self.literal_layers):
            for goal in self.goal:
                if goal not in goals_visited and goal in layer:
                    goals_visited.append(goal)
                    cost.append(i)
    ...
    def h_setlevel(self):
        ...
        self.fill()
        for index,values in enumerate(self.literal_layers):
            goals = [True if goal in values else False for goal in self.goal]
            if all(goals):
                goalmutex = [False if not values.is_mutex(goalA, goalB) else True for goalA in self.goal for goalB in self.goal]
                if not any(goalmutex):
                    return index
        return None
        raise NotImplementedError

...
# evaluvation function assingning scores to terminal states of the game
def evaluvation_function(board):
...
def minimax_value(board, white_turn, search_depth, alpha, beta):
    # TODO
    # If depth is zero call the evaluvation function to return the value belongs to the current board
    if search_depth == 0:
        return evaluvation_function(board)
    move_list = generate_legal_moves(board, white_turn)
    # If there are no legal moves for a player move to the opposite players tree part cll the function recursively
    if len(move_list) == 0:
        return minimax_value(board, not white_turn, search_depth - 1, alpha, beta)
    # if it is white turn go with thi part
    if white_turn:
        value = float("-Inf") # assign initial value to negative infinity
        for move in move_list:
            newBoard = play_move(copy.deepcopy(board),move,white_turn) #for each move in valid move list generate its board
            v = minimax_value(newBoard,not white_turn, search_depth - 1, alpha, beta) #compute v value by calling the minimax function recursively with calling the opposite part of whites turn
            value = max(value, v) #select the maximum between v and value and assign to value
            if value > beta: # if value is greater than beta quit this part of search and return the value to its parents node
                break
            alpha = max(alpha,value) #set maximum of value and alpha to alpha
    # if blacks turn go with this part of loop
    else:
        value = float("inf")  # assign initial value to infinity
        for move in move_list:
            newBoard = play_move(copy.deepcopy(board),move,white_turn) #for each move in valid move list generate its board
            v = minimax_value(newBoard,not white_turn, search_depth - 1, alpha, beta) #compute v value by calling the minimax function recursively with calling the opposite player part
            value = min(value, v) #select the minimum between v and value and assign to value
            if value < alpha: # if value is lesser than alpha quit this part of search and return the value to its parents node
                break
            beta = min(beta,value) #set minimum of value and beta to beta
    return value

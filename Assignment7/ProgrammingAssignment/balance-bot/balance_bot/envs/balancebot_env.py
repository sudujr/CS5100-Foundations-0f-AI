...
class BalancebotEnv(gym.Env):
    ...
    def _assign_throttle(self, action):
        # made the robot to move and fall with certain velocity or fall directly based on weighted probability
        x = [0.1,0.05,0.025,0.01,0]
        l = random.choices(x,weights=[50,20,10,5,10],k=1)
        dv = l[0]
        deltav = [-25.*dv,-20.*dv, -10.*dv,-5.*dv, -2.*dv, -0.1*dv, 0, 0.1*dv, 2.*dv,5.*dv, 10.*dv, 20.*dv,25.*dv][action]
